...
namespace JoyMoe.Common.Data.Dapper.Tests
{
    public class TranslatorTests
    {
        ...
        public void TranslateStringEquals()
        {
            ...
            (clause, parameters) = Translate(s => string.Equals(s.LastName, "Sophia", StringComparison.InvariantCultureIgnoreCase));
            Assert.Equal("(UPPER(\"LastName\") = UPPER(@__p0))", clause);
            Assert.NotNull(parameters);
            Assert.Equal("Sophia", parameters!.Get<string>("@__p0"));
        }
        ...
        public void TranslateLogicality()
        {
            ...
            (clause, parameters) = Translate(s => !values.Contains(s.Id) && (s.FirstName.StartsWith("Caro") || s.LastName == "Sophia"));
